 ...
 const Dashboard = () => {
  ...
  const connectInstagram = async () => {
    const facebookExist = user.connect.find(
      (target: any) => target.social === "facebook"
    );
    if (!facebookExist) {
      setShowModal(true);
      setMessage(
  ...
  const logoutTwitter = async (e: any) => {
    ...
    setDeleting(true);
    const con = user.connect.find((target: any) => {
  ...
  const logoutInstagram = () => {
    ...
    const instagramDetails = user?.connect?.find((target: any) => {
  ...
  const logoutFacebook = () => {
    ...
    const facebookDetails = user?.connect?.find((target:any) => {
  ...
  const logoutLinkedin = () => {
    ...
    const linkedinDetails = user?.connect?.find((target: any) => {
          ...
          <nav className="relative">
            ...
            <div>
              <div
                ref={ref2}
                id="menu"
                className="rounded-sm absolute hidden flex-col items-center self-end mt-14 space-y-6  bg-owhite w-full sm:self-center  drop-shadow-md"
              >
                <a
          ...
          <div className={loading ? "hidden" : "flex  "}>
            ...
            >
              ...
              <div className="flex justify-center space-x-4 max-w-{500px} w-full md:space-x-10  w-full md:w-auto items-center">
                {socials.some((e: any) => e.type === "twitter") === false &&
                connecting ? (
                  <CircularProgress />
                ) : (
                  socials.some((e: any) => e.type === "twitter") === false && (
                ...
                {socials.some((e: any) => e.type === "facebook") === false &&
                loadingFacebook ? (
                  <CircularProgress />
                ) : (
                  socials.some((e: any) => e.type === "facebook") === false && (
                ...
                {socials.some((e: any) => e.type === "instagram") === false &&
                loadingInstagram ? (
                  <CircularProgress />
                ) : (
                  socials.some((e: any) => e.type === "instagram") === false && (
                ...
                {socials.some((e: any) => e.type === "linkedin") === false &&
                loadingLinkedin ? (
                  <CircularProgress />
                ) : (
                  socials.some((e: any) => e.type === "linkedin") === false && (
