...
const reducer = (state, action) => {
    switch (action.type) {
        ...
        case "addPicture":
            const updatedMedia = addPicture(action.fileType, action.file, action.index, state);
            return updatedMedia;
        case "removeMedia":
...
const addItem = (item, index, state) => {
    if (state.value.length == 0) {
        ...
        const newValue = state.value.map((obj, i) => {
            if (i === index) {
                return Object.assign(Object.assign({}, obj), { text: item });
            }
            else {
                return obj;
            }
...
const addPicture = (fileType, file, index, state) => {
    const newValue = state.value.map((obj, i) => {
        if (i === index) {
            ...
            return Object.assign(Object.assign({}, obj), { media: allMedia });
        }
        else {
            return obj;
        }
...
const removeMedia = (media, index, state) => {
    const newValue = state.value.map((obj, i) => {
        if (i === index) {
            ...
            return Object.assign(Object.assign({}, obj), { media: newMedia });
        }
        else {
            return obj;
        }
